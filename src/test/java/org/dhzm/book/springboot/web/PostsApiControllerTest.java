...
/*
    Api Controller
    - @WebMvcTest를 사용하지 않음. -> @WebMvcTest의 경우 JPA 기능이 작동하지 앟기 때문
    - Controller와 ControllerAdive등 외부 연동과 관련된 부분만 활성화되니 JPA 기능까지 한번에 테스트 할 때에는
        @SpringBootTest와 TestRestTemplate을 사용

    @WithMockUser(roles="USER")
    - 인증된 모의 사용자를 만들어서 사용한다.
    - roles에 권한을 추가할 수 있음
    - 어노테이션으로 인해 ROLE_USER권한을 가진 사용자가 API를 요청하는 것과 동일
    --> MockMvc에서만 작동
